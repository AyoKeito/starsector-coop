...
`netfault{mode, seconds, lossPercent}` makes **this** instance stop hearing its peer, so a link drop can be reproduced when you want one instead of by freezing a JVM from outside. `mode: "discard"` reads every inbound byte off the socket and throws it away, TCP and UDP; `mode: "loss"` drops `lossPercent` of inbound datagrams and leaves TCP alone; `mode: "clear"` ends it now. `seconds` is 1-180 and the fault expires by itself. **Outbound is untouched in every mode** — this side keeps writing and the peer keeps hearing it, which is the point: the failure worth reproducing is bytes the sender's TCP stack considers delivered that the receiver never applied, which is what the reliable-delivery layer heals and what suspending the peer process does *not* produce. A symmetric outage is the verb run on both instances. Nothing refreshes the peer's inbound silence clock while a discard runs, so the link dies the ordinary way at 15 s. `status` carries a `netfault` block (`active`, `mode`, `remainingSeconds`, `discardedBytes`, `droppedDatagrams`) so a fault you forgot about cannot pass for a real bug.
