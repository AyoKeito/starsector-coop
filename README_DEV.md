...
On screen, both clients draw a coop line under the vanilla fps overlay (top-right): `HOST · session active`, `GUEST · session active · paused by host`, and on the guest the clock drift in whole game-hours once it reaches an hour. `-Dcoop.hud.disable=true` turns it off. The guest's New Game dialog shows the join target on its Continue option and pins seed, sector size and star age; `-Dcoop.sectorSize=small|normal` and `-Dcoop.sectorAge=young|average|old|mixed` override the panel defaults on both roles when a non-default world is wanted. `-Dcoop.clock.disable=true` turns off the Phase 7c clock reconciler (guest only). The Phase 20 networking properties (`coop.hostPort`, `coop.connectHost`, `coop.connectPort`, `coop.password`, `coop.maxGuests`, `coop.reconnectGraceSeconds`, `coop.portMapping`, `coop.debug.wiretap`, `coop.debug.wiretapSample`) are documented with their defaults in `docs/player/INSTALL.md`, "Every setting".

`-Dcoop.debug.allyPullIn=true` (plus `-Dcoop.debug.allyPullInDropShield=true` for its second run) arms the ally pull-in spike: the partner's mirror is created joinable and the host's battle eject only logs, so the engine is allowed to drag the mirror into a battle and the `Coop SPIKE ally pull-in` lines record what it did with it. It leaves the partner's fleet open to autoresolve damage, so it is a two-instance dev instrument and nothing else; runbook in `tmp_ff_analysis\ally-pullin-spike\RUNBOOK.md`.
...
| verb | shape |
| --- | --- |
| `status` | adds `pause`: `blockingScreenOpen` on both roles, plus `hostIntent` / `guestIntent` / `guestKeyIntent` / `guestScreenIntent` / `eitherInCombat` / `effective` on the host. When an advance stalls, that block says which term of the coordinator's OR is holding the clock. Also `reliable`: `unacked` (actions this side has written and not had acknowledged — the queue the replay walks), `duplicatesDropped` (replayed messages this side had already applied) and `appliedSeqs` (how many applied sequence numbers it still remembers). That trio is the whole check a `netfault` run makes, and none of it is visible from outside otherwise. |
...
| `entities` | everything in one location — `system` (id, name, or `"hyperspace"`; default is wherever the local player fleet is) and `kinds` (`planet`, `station`, `jumpPoint`, `relay`, `base`, `fleet`, `other`). Rows carry `id`, `name`, `kind`, `type`, `faction`, location-local `x`/`y`, sorted `tags`, `orbitFocus`, `hidden`, `discoverable` and `marketId`, capped at 300 with a `truncated` flag. **Asteroids, orbital junk, ring bands and non-debris terrain are dropped by default** and `clutterExcluded` says so: in Corvus and Askonia those four ate the entire 300-row cap and the system's gate never appeared in a list of everything in the system. Terrain is judged by its terrain id rather than its class, so a debris field stays — it is a salvage target that happens to be implemented as terrain. Passing `kinds` at all turns the filter off (asteroids are only reachable as `other`, and filtering out what you asked for is not an answer), as does `includeClutter: true`. **This is how you get a hidden base's id.** `base` is decided by the market being hidden — the same discriminator the replicator uses — and is tested before `station`, because a hidden base is a station too. `teleport` never refused those ids; nothing emitted them. `landmarks` covers worldgen one-offs by tag and `markets` covers the economy, and a runtime-minted base with a market outside the economy index is in neither. The `marketId` on a base row is the key to look up in `status`'s `baseMarketIds` to see whether the guest has paired it. |
| `intel` | the local player's intel entries: `class`, `title`, sorted `tags`, `isNew` / `isEnding` / `isEnded` / `important` / `hidden`, `factionId`, and an `extra` object — `progress` / `maxProgress` / `factors` for anything descending from `BaseEventIntel`, plus whatever the mod's own pages report through `CoopIntelFacts` (the expedition warning's kind/status/target/ETA, the stats page's team totals). `filter` is a case-insensitive substring of class or title, `limit` defaults to 50. The title comes from `getSmallDescriptionTitle()`, which is what `BaseIntelPlugin` delegates to `getName()`; `getName()` itself is `protected` and the sandbox forbids the reflection that would open it. A separate `hostileActivity` block answers `present` / `progress` / `factors` straight off `HostileActivityEventIntel.get()`, so "no Hostile Activity on the guest" is a positive answer rather than the absence of a row — the entry can be present and hidden, and a scan would call that absent. |
| `feed` | the last 200 co-op feed lines this instance posted, oldest last, `limit` default 20. Each is `atMillis` / `kind` / `text` / `color` (`#RRGGBB`, or empty for the feed default). **It survives the session ending** — the ring is cleared only by a full reset, not by `endSession()` — which is the point: what the guest's screen said as the link died is asked after the link died. It is a separate ring from the intel page's twenty-row event list, not a widening of it; the page is a screen and twenty rows is what fits, this is a transcript. `sessionEnded` distinguishes "there was a session and it is over" from "there has never been one", which `role: NONE` alone cannot. |
| `screen` | what the UI is doing: `state`, `paused`, `uiAvailable`, `dialogOpen`, `menuOpen`, `fastForward`, `interactionDialog` (the *plugin's* class — the engine's dialog class is obfuscated and says nothing) with `interactionTargetId`/`interactionTargetName`, `coreTab`, `coopDialog` (`kind` / `title` / `shown` for the reconnect, desync, lobby or connecting dialog that is requested, in the arbiter's precedence order — "requested but not shown" is a real state and one you cannot otherwise see), and the same `pause` block `status` carries. Every field degrades on its own; a UI that will not answer one accessor still reports the rest. |
...
`netfault{mode, seconds, lossPercent, delaySeconds}` makes **this** instance stop hearing its peer, so a link drop can be reproduced when you want one instead of by freezing a JVM from outside. `mode: "discard"` reads every inbound byte off the socket and throws it away, TCP and UDP; `mode: "loss"` drops `lossPercent` of inbound datagrams and leaves TCP alone; `mode: "clear"` ends it now. `seconds` is 1-180 and the fault expires by itself. **Outbound is untouched in every mode** — this side keeps writing and the peer keeps hearing it, which is the point: the failure worth reproducing is bytes the sender's TCP stack considers delivered that the receiver never applied, which is what the reliable-delivery layer heals and what suspending the peer process does *not* produce. A symmetric outage is the verb run on both instances. Nothing refreshes the peer's inbound silence clock while a discard runs, so the link dies the ordinary way at 15 s. `status` carries a `netfault` block (`active`, `mode`, `remainingSeconds`, `discardedBytes`, `droppedDatagrams`, `armed`, `startsInSeconds`) so a fault you forgot about cannot pass for a real bug.

`delaySeconds` (0-60, default 0) **arms** the fault instead of starting it, because the tester is a human at two game windows and a fault that lands while they are still in a menu proves nothing. An armed fault exists, is reported, and drops nothing: `shouldDiscardTcp` and `shouldDropDatagram` both answer false until it flips, and `active` stays false while `armed` is true (the two are never both set; `active || armed` is "there is a fault"). The log says `Coop netfault discard armed, starts in 5 s for 40 s` when it is scheduled and the ordinary `Coop netfault discard for 40 s started` when it flips, which happens in the per-poll lifecycle step under the transport's lifecycle lock. `remainingSeconds` reads the requested duration while armed rather than the distance to `endsAtMillis`, so a 40 s fault armed 5 s out never claims to be 45 s long. `mode: "clear"` while armed cancels it and answers `{cleared: true, wasArmed: true}`, which is the only way to tell "cancelled before it bit" from "ran and dropped nothing".

The countdown is on the **HUD**, not the feed: `CoopLinkHud` draws a second line under (or above, in the bottom corners) the link readout in orange, reading `NET FAULT discard 40s IN 5` while armed and `NET FAULT discard 37s LEFT` while running, and nothing at all otherwise. Per-second writes are exactly what makes the vanilla text panel flash, which is why this is a HUD line and not a feed one. The feed still gets one line when the outage starts (`Co-op test: net fault (discard) active for 40 s.`) and one when it ends (`Co-op test: net fault ended.`), so the `feed` verb's record shows the run afterwards. The HUD reads a per-frame cached copy of the status off `CoopNetPump`, never `CoopNetService` directly, so the render pass cannot block behind a socket read.

`save{force?}` takes this instance's own vanilla autosave. That is an F5 in everything but the keypress: the engine fires `ModPlugin.beforeGameSave()` / `afterGameSave()` around `CampaignUIAPI.autosave()`, and the mod's `afterGameSave` is where `CoopSaveCheckpoint.notifyLocalGameSaved` sends the guest its `SAVE_CHECKPOINT` — so a host `save` produces the coordinated pair. `CoopSaveIndex.beginCoopAutosave()` brackets it, so the row lands in the save index tagged as an autosave. It is refused on the **guest** without `force: true`, because a guest save the host did not order is aligned with no host save and is not what a rejoin loads; and it is refused whenever a dialog is open or the campaign is not on screen, because `autosave()` is silently a no-op in both cases and a `requested: true` for a save that never happened is worse than a refusal. It runs inline on the campaign thread — the bridge dispatches inside its own `advance()` — so there is nothing to wait for.

`mark{text}` writes one `Coop MARK <text>` INFO line into this instance's log and returns the `atMillis` it was written at. Two logs from two JVMs have no shared reference point; a mark on both sides at the top of a step gives every line after it a known offset. Needs no campaign and no session, so it works at the title screen. Newlines are refused rather than stripped: a mark that silently became two lines would break the grep it exists for.

`memory{scope, key, entityId?, value?, expireDays?}` reads or writes one campaign-memory key. `scope` is `"global"` for sector memory — the `$global.` namespace rules.csv writes against — `"player"` for the player fleet's memory, or `"entity"` with an `entityId` resolved exactly the way `teleport`'s entity mode resolves one. The key takes any spelling: `canScanGates`, `$canScanGates` and `$global.canScanGates` all mean sector memory's `"$canScanGates"`, because the `global.` part names *which memory*, not the key; a `global.` prefix in the other two scopes is refused rather than written to a key nobody reads. With no `value` it reads and answers `{scope, key, present, value, type}`, where `type` is the stored object's Java class; with one it writes and answers `{scope, key, before, after}`. Booleans stay booleans and strings stay strings, a number lands as the `Float` `MemoryAPI` stores (a rules comparison reads floats), and `expireDays` goes through `MemoryAPI.set(key, value, expire)`.

Both roles, reads and writes alike — this is a test harness and half its use is putting the two instances into deliberately unequal states. Every write leaves a WARN `Coop bridge memory write scope=… key=… before=… after=…` line in the log, because a flag set by hand is exactly what makes a later "desync" unexplainable an hour later. The case that forced the verb: the vanilla "Scan the Gate" option is gated on `$global.canScanGates`, which nothing but the At the Gates story chain ever sets, so without a way to set it on the host neither the flag's replication to the guest (`CoopSkeletonMutationWatcher` / `WORLD_DELTA` `GATE_ACTIVATED`) nor the guest's own scan can be exercised at all. It is an `ss_act` verb even when it reads: the MCP client's dropped-socket retry is keyed by verb name, one name cannot be both retry-safe and not, and a replayed write would report a `before` that never existed.
...
2. Update `CHANGELOG.md`. Verify every file in `docs/player/` against the code (quoted strings,
   numbers, behaviour, cross-references) and fix it in place. Update `docs/CODEBASE_MAP.md` and move
   its "Last checked" line to the new version. These docs are current instructions, not a record of
   what changed: edit wrong text and delete stale text, never append a new paragraph beside an old
   one.
