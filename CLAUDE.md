...
- `docs/roadmap.html` is generated. Run `node docs/roadmap_gen.js` after editing the plan, and hand-refresh the banner, ATTENTION entries and bucket notes in `docs/roadmap.data.json` on every phase landing (outstanding items only; history stays in the ledger). Never edit the HTML.
- `docs/COOP_MP_DESIGN.md` is the design rationale; `docs/CONNECTIVITY.md` is the player-facing networking guide.
