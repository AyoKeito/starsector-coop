...
- [x] Run the automated 7-item portion of the 12c smoke via MCP end-to-end; record results in the 12c smoke step; leave the 3-item manual residue listed there. *(**RUN 2026-08-25, all 7 PASS** — full results + 3 findings recorded in the 12c smoke step. Tooling nits found during the run, none blocking: `ss_diff` should exclude the per-instance `role`/`engineId` fields and key the two player fleets consistently; the `survey` verb's single-system lookup rejects ids its own `all` enumerator emits; the `ability` verb cannot deactivate a toggled ability (`activate()` re-arms it — needs an `off` arg if ever wanted).)*
