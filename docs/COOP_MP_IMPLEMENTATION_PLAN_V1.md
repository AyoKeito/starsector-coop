...
- [ ] Run a two-instance smoke test: confirm the guest shows the same NPC fleets as the host in a shared system (count, factions, rosters), that fleets the host despawns disappear on the guest, and that the guest's `starsector.log` never logs an independently-spawned NPC fleet.
...
- [ ] Run two-instance smoke test by clicking the same market/fleet from both clients. *(Manual in-game step; not run in this headless session.)*
...
2. **Market capture covers only commodities + mothballed ships.** `captureMarketContents` reads submarket commodity stacks and ships; weapon/fighter stock and the hireable officer/merc pool (the model already supports `WEAPON/FIGHTER/OFFICER/MERC`) are not captured, so those diverge between clients.
...
- [ ] **Market capture:** extend the submarket loop to weapons/fighters; capture hireable officers/mercs; guest renders host stock for all kinds; `MARKET_TXN` apply covers the new kinds.
