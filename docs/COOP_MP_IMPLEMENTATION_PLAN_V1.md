...
- [ ] Two-instance smoke with the latency lever: both dock the same market inside the widened window — the loser's dialog closes with the message; the log shows exactly one claim/reject pair. *(Residue, 2026-08-24 — user-driven, not yet run. Launch the HOST with `-Dcoop.debug.interactionDelayMs=1500`; both players dock the same market within that window. Loser's log: exactly one `Coop net GUEST outbound INTERACTION_CLAIM`, one `Coop interaction rejected entityId=<id> already_claimed_by:<player>; closing the local dialog`, one `Coop force-closing rejected interaction dialog entityId=<id>`, and no second claim for that entity. Host log: one `Coop debug delaying INTERACTION_CLAIM seq=<n> by 1500ms` then one `Coop interaction claim rejected entityId=<id>`. Fail conditions: repeated claim/reject pairs, a `Failed to force-close rejected interaction dialog` warn, or `Coop force-closing a rejected dialog while core tab ... is open`.)*
