...
| 18–19 | NOT BUILT | V1. 18 rescoped 2026-08-20 and shrank (interaction-gate WAN race; lock system cancelled — see its banner). 19 is the final sign-off and runs last, after 20. |
...
- [ ] Smoke test (cheap since the Ziggurat preset): deliberate wipe in both directions — the vanilla shuttle dialog fires; the partner sees the banner; the partner's log shows no 0-roster commit and no despawn/recreate churn; skills/officers/rep survive. *(Residue 2026-08-20: the code landed unsmoked; run this in the next two-instance session. Log markers to watch: the wiped client's `Coop local fleet was destroyed;` line, the partner's `Coop <name>'s fleet was destroyed - respawned at …` and `Coop player mirror kept its last roster` — and, on the partner, the absence of any `roster refreshed to 0 of 0 ship(s)` against the `player:` mirror.)*
