...
- [ ] **Build task D — survey replication (gap 5):** `WORLD_DELTA(SURVEY)` + ruins-explored kind on the both-sides skeleton poll, ordinal max-wins apply via `Misc.setFullySurveyed`, `remote_survey` reclassified LOCAL.
- [ ] Record the accepted divergences from the verification pass (contacts/contact-board/person-bounties local, per-player offer scaling, multi-module ships, merc-level roll, interdiction sensorRangeMod fidelity, 30-day writeReplace stock clear, remote-survey once-per-system flag per player) in `docs/starsector-runtime-limitations.md`.
...
- [ ] Commit per build task (`feat: apply ability world effects + guest objective capture`, `feat: full-fidelity market capture`, `feat: bar pool enumeration + guest injection`, `feat: replicate survey levels and ruins exploration`).
