...
| 19 | NOT BUILT | V1. The final sign-off; runs last, after 20. |
...
| 29 | NOT BUILT | **Pulled into V1 2026-08-20** (user decision) after the jumpy-mirror root cause was verified live during the perf-debug session — see the Phase 29 banner for the two measured failure modes. M1 (interpolation) slots after 18; M2 (cadence) needs 20 and slots after it. The M1 partner-mirror scope question is SETTLED: included — the host↔guest orbit pair was the worst live offender. **M1 mechanism revised 2026-08-24** after the deep-research pass (see the research banner): buffered snapshot interpolation + kinematic drive replaces pure dead reckoning; the datagram envelope gains epoch + send-time fields as an M1 prerequisite (pulled forward from 20.1). |
...
- [ ] **M1 prerequisite:** datagram envelope `epoch` + `sentGameTime` + redundant-sample packing + receiver watermark. Commit `feat: sequenced, stamped state datagrams`.
- [ ] **M1:** implement `CoopMotionInterpolator` (buffer, Hermite, game-clock cursor, ladder, snap rules), convert `CoopFleetMirror` to kinematic drive for both the player-mirror and NPC paths; tune the snap radii on the shaped harness (100 ms ± jitter + 2% loss — the clean-link baseline is already established per the 2026-08-20 banner); acceptance is visually continuous motion with no oscillation. Commit `feat: mirror snapshot interpolation`.
