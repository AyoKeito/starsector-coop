...
- Modify `src/main/java/coop/net/CoopMessages.java` (`senderId`; datagram `epoch` already landed with Phase 29 M1; new types `UDP_PROBE`, `PATH_PROBE`, `LINK_STATUS`, `SESSION_RESUME_REQUEST/ACCEPT/REJECT`)
...
- [ ] Add `senderId` to the TCP envelope and the datagram envelope (`epoch` + `sentGameTimeMillis` already landed with Phase 29 M1); bump nothing else (exact-install handshake parity means both ends always speak the same version — no wire versioning needed).
