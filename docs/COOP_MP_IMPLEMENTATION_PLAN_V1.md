...
| 14b | **BUILDING** | Created 2026-08-19 out of a decompile pass that corrected two Phase 14 conclusions. Code + unit tests landed; every acceptance box is unchecked pending the smoke session, which also decides primary vs fallback pursuit model. |
...
- [ ] **Customs stop.** Transponder-off guest close to a Hegemony patrol that can see it: `Coop DIALOG_BEGIN sent (customs, transponder-off guest)`. Same patrol, guest dark and far away: no line.
