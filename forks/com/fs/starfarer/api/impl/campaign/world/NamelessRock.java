 ...
 * Original source     : starfarer.api.zip!/com/fs/starfarer/api/impl/campaign/world/NamelessRock.java
 * Compiled into       : mods/coop/jars/coop-forks.jar (classpath-prepended ahead of starfarer.api.jar).
 * RNG replacement: same as GateHaulerLocation - generate(SectorAPI) (and the addOnslaughtMkI /
 *   createSensorArrayInClosestSystem it calls) place a one-time deep-space system off the shared,
 *   drifted StarSystemGenerator.random. In a coop session, swap in an independent topic-keyed
 *   CoopRandom stream for the duration of generate(), restored in finally; nested calls inherit it.
 * Outside a coop session behaviour is byte-for-byte vanilla.
 * ==========================================================================================
 */
...
public class NamelessRock {
	...
	public void generate(SectorAPI sector) {
		// COOP FORK: run this one-time deep-space placement on an independent session-seeded RNG
		// stream (immune to upstream drift of the shared StarSystemGenerator.random), restored in
		// finally. Nested addOnslaughtMkI / createSensorArrayInClosestSystem calls inherit it.
		java.util.Random coopPrevRandom = StarSystemGenerator.random;
		if (coop.rng.CoopRandom.isCoopSession()) {
			StarSystemGenerator.random = coop.rng.CoopRandom.of("NamelessRock");
		}
		try {
...
//		}
	...
	protected void createSensorArrayInClosestSystem(StarSystemAPI rockSystem) {
		...
		for (StarSystemAPI curr : Global.getSector().getStarSystems()) {
			...
			if (curr.hasTag(Tags.THEME_HIDDEN)) continue;
			if (curr.hasTag(Tags.THEME_SPECIAL)) continue;
			if (curr.hasTag(Tags.TEMPORARY_LOCATION)) continue;
			if (curr.hasTag(Tags.SYSTEM_ABYSSAL)) continue;
