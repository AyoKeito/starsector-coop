 ...
 * Original source     : starfarer.api.zip!/com/fs/starfarer/api/impl/combat/threat/DisposableThreatFleetManager.java
 ...
 * pickCurrentSpawnLocation() with its own copy of the same player-distance loop (over systems tagged
 * SYSTEM_CAN_SPAWN_THREAT). Forking only the base would leave the abyss empty for a guest exploring
 * it alone, so the override gets the identical treatment.
 *
 ...
 *   - line ~165 : pickCurrentSpawnLocation()      - candidate distance = min over {player, presence}
 *   - line ~180 : pickCurrentSpawnLocation()      - the "stick with current system" fallback, same min
 *
...
public class DisposableThreatFleetManager extends DisposableFleetManager implements CurrentLocationChangedListener {
	...
	protected StarSystemAPI pickCurrentSpawnLocation() {
		...
		for (StarSystemAPI system : Global.getSector().getStarSystems()) {
			...
			// so an abyssal system the guest is exploring is eligible to become currSpawnLoc on its own.
